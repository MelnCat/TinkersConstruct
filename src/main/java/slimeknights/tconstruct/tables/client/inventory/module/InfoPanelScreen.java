...
public class InfoPanelScreen extends ModuleScreen {
  ...
  public void renderTooltip(GuiGraphics graphics, int mouseX, int mouseY) {
    ...
    if (this.hasTooltips() && mouseX >= this.guiRight() - this.border.w - this.font.width("?") / 2 && mouseX < this.guiRight()
      ...
      graphics.renderTooltip(Minecraft.getInstance().font, this.font.split(Component.translatable("gui.tconstruct.general.hover"), 150), mouseX - 155, mouseY);
    }
    ...
    graphics.renderTooltip(Minecraft.getInstance().font, lines, mouseX, (mouseY - lines.size() * this.getScaledFontHeight() / 2));
  }
  ...
  protected void renderBg(GuiGraphics graphics, float partialTicks, int mouseX, int mouseY) {
    ...
    if (this.hasTooltips()) {
      graphics.drawString(font, "?", guiRight() - this.border.w - this.font.width("?") / 2, this.topPos + 5, 0xff5f5f5f);
    }
    ...
    this.slider.texture = BACKGROUND_IMAGE;
